...
const randomWord = () => LOREM_IPSUM_WORDS[Math.random() * LOREM_IPSUM_WORDS.length];

const genericTestThumbnails = [];
for (let i = 1; i < 4; ++i) {
  genericTestThumbnails.push(`${process.env.PUBLIC_URL}/testimg/test${i}`);
}
...
for (let i = 3; i < TEST_PLANT_COUNT; ++i) {
  ...
  plants.push({
    ...
    thumbnail: genericTestThumbnails[(Math.random() * 30) % 3],
    tags: Math.random() < .5 ? [] : GENERIC_TAGS[(Math.random() * GENERIC_TAGS.length).toFixed(0)],
...
const TEST_DATA = { plants, rooms, LOREM_IPSUM };
